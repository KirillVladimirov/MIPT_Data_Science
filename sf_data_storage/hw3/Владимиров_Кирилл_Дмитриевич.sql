...
SELECT
    ...
    coalesce(SUM(CAST(REPLACE(t.list_price, ',', '.') AS NUMERIC)), 0) AS total_amount,
...
WITH transaction_stats AS (
    SELECT
        ...
        coalesce(SUM(CAST(REPLACE(t.list_price, ',', '.') AS NUMERIC)) OVER (PARTITION BY c.customer_id), 0) AS total_amount,
...
--Решение для максимальной суммы.
SELECT
    c.customer_id,
    c.first_name,
    c.last_name,
...
--|-----------|----------|---------|------------|
--|2183		  |Jillie	 |Fyndon   |19071.32    |

--Решение для минимальной суммы.
SELECT
    c.customer_id,
    c.first_name,
    c.last_name,
...
-- Определим максимальный интервал
, MaxInterval AS (
    SELECT MAX(interval_days) AS max_interval
    FROM IntervalsWithDiff
...
FROM
    ...
    MaxInterval m
WHERE
    i.interval_days = m.max_interval
ORDER BY
    i.interval_days DESC;
--|customer_id|first_name|last_name|job_title      |interval_days|
--|-----------|----------|---------|---------------|-------------|
